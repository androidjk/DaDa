...
public class ShoppingAdapter extends BaseAdapter {
    ...
    public void setShoppingCartBeanList(List<Cark> carkList) {
        this.carkList = carkList;
    ...
    @Override
    public int getCount() {
        return carkList == null ? 0 : carkList.size();
    }

    @Override
    public Object getItem(int position) {
        return carkList.get(position);
    }
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            convertView = LayoutInflater.from(context).inflate(R.layout.dingdantiao, parent, false);
            holder = new ViewHolder(convertView);
            convertView.setTag(holder);
        } else {
            holder = (ViewHolder) convertView.getTag();
        }
        final Cark cark = carkList.get(position);
        boolean choosed = cark.isChoosed();
        if (choosed) {
            holder.ckOneChose.setChecked(true);
        } else {
            holder.ckOneChose.setChecked(false);
        }
        String attribute = cark.getAttribute();
        if (!StringUtil.isEmpty(attribute)) {
            holder.tvCommodityAttr.setText(attribute);
        } else {
            holder.tvCommodityAttr.setText(cark.getType() + "");
        }
        holder.tvCommodityName.setText(cark.getName());
        holder.tvCommodityPrice.setText(cark.getPrice() + "");
        ...
        holder.ckOneChose.setOnClickListener(
                new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        cark.setChoosed(((CheckBox) v).isChecked());
